...
open class ObjectList(
    capacity: Int,
    val typeSize: Int = Byte.SIZE_BYTES
) : FastCollection {

    protected var buffer = FastBuffer(capacity * typeSize)
    ...
    fun addBoolean(value: Boolean) = addInt(if (value) 1 else 0)

    fun addShort(value: Short) = addInt(value.toInt())
    ...
    fun copyTo(dest: ObjectList, srcIndex: Int, destIndex: Int, size: Int) {
    ...
    protected fun ensureSize() {
        if (size == this@ObjectList.capacity) {
            resize(this@ObjectList.capacity * 2)
        }
    }

    protected fun ensureCapacity(i: Int) {
        if (i >= this@ObjectList.capacity || i < 0) {
            throw IllegalArgumentException("Index is out of bounds! i=$i capacity=${this@ObjectList.capacity}")
        }
    }
