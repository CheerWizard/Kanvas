 ...
 */
fun GPUDevice.onUncapturedError(handler: (GPUUncapturedErrorEvent) -> Unit) {
    this.asDynamic().addEventListener("uncapturederror") { event: dynamic ->
        handler(event.unsafeCast<GPUUncapturedErrorEvent>())
    }
}
