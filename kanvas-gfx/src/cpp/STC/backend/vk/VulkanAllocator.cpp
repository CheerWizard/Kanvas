...
VKAPI_ATTR void* VKAPI_CALL vk_allocate(void* userData, size_t size, size_t alignment, VkSystemAllocationScope scope) {
    return static_cast<stc::VulkanAllocator*>(userData)->allocate(size, alignment, scope);
}

VKAPI_ATTR void VKAPI_CALL vk_free(void* userData, void* memory) {
    static_cast<stc::VulkanAllocator*>(userData)->free(memory);
}
...
VKAPI_ATTR void* VKAPI_CALL vk_reallocate(void* userData, void* oldMemory, size_t size, size_t alignment, VkSystemAllocationScope scope) {
    return static_cast<stc::VulkanAllocator*>(userData)->reallocate(oldMemory, size, alignment, scope);
}
...
VKAPI_ATTR void VKAPI_CALL vk_allocate_notification(void* userData, size_t size, VkInternalAllocationType type, VkSystemAllocationScope scope) {
    static_cast<stc::VulkanAllocator*>(userData)->allocateNotification(size, type, scope);
}
...
VKAPI_ATTR void VKAPI_CALL vk_free_notification(void* userData, size_t size, VkInternalAllocationType type, VkSystemAllocationScope scope) {
    static_cast<stc::VulkanAllocator*>(userData)->freeNotification(size, type, scope);
}

stc::VulkanAllocator::VulkanAllocator() {
...
stc::VulkanAllocator& stc::VulkanAllocator::getInstance() {
    static VulkanAllocator instance;
    return instance;
}

void stc::VulkanAllocator::New(VkInstance instance, VkPhysicalDevice physicalDevice, VkDevice device) {
...
void stc::VulkanAllocator::Delete() const {
    vmaDestroyAllocator(allocator);
}

void* stc::VulkanAllocator::allocate(size_t size, size_t alignment, VkSystemAllocationScope scope) {
...
void stc::VulkanAllocator::free(void *address) {
...
void* stc::VulkanAllocator::VulkanAllocator::reallocate(void *oldAddress, size_t size, size_t alignment, VkSystemAllocationScope scope) {
...
void stc::VulkanAllocator::allocateNotification(size_t size, VkInternalAllocationType type, VkSystemAllocationScope scope) {
    LOG_INFO(TAG, "allocateNotification: size=%d, type=%d, scope=%d", size, type, scope);
}

void stc::VulkanAllocator::freeNotification(size_t size, VkInternalAllocationType type, VkSystemAllocationScope scope) {
