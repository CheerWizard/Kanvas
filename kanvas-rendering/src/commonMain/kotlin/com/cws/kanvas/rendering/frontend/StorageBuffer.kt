...
class StorageBuffer<T : INativeData>(
    ...
    info = BufferInfo(
        ...
        usages = BufferUsage.STORAGE_BUFFER.value,
        size = (factory().sizeBytes(MemoryLayout.STD430) * size).toLong(),
        isStatic = false,
    ),
) {
    val list: NativeDataList<T> = NativeDataList(MemoryLayout.STD430, size, factory)
}